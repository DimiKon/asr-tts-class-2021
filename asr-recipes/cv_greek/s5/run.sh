...
# Data preparation
if [ $stage -le 1 ]; then
  bash local/greek_data_prep.sh 
  utils/combine_data.sh data/all data/train data/dev
  bash local/prepare_lm.sh --data data/train --locdata data/local/lm
  bash local/prepare_dict.sh data/local/lm data/local/dict_nosp

  utils/prepare_lang.sh data/local/dict_nosp \
    "<UNK>" data/local/lang_tmp_nosp data/lang_nosp

  bash local/greek_format_lms.sh --src-dir data/lang_nosp data/local/lm
...
if [ $stage -le 7 ]; then
  ...
  bash local/greek_format_lms.sh --src-dir data/lang data/local/lm
